...
export default function CardGame() {
  ...
  // Update cards when shuffled
  useEffect(() => {
    console.log("Cards shuffled!", blackCard, whiteCards); // To check when the cards are updated
  }, [blackCard, whiteCards]);
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {whiteCards.map((card) => (
          <Card key={card} content={card} isBlack={false} />
        ))}
      </div>
      <motion.button
